...
class ProductAdapterPAcc(private val context: Context, private val list: List<Product>) :
    Adapter<ProductAdapterPAcc.ProductHolderPAcc>() {
    inner class ProductHolderPAcc(val binding: SellerProductItemLyBinding) :
        ViewHolder(binding.root)

    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): ProductHolderPAcc {
        val binding =
            SellerProductItemLyBinding.inflate(LayoutInflater.from(context), parent, false)
    ...
    override fun onBindViewHolder(holder: ProductHolderPAcc, position: Int) {
        ...
        holder.binding.returnPolicyItem.text = "Return Policy : " + currentProduct.returnPolicy
